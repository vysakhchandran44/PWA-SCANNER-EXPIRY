...
function buildMasterIndex(masterData) {
  ...
  for (const item of masterData) {
    ...
    // Store GTIN-14 padded version
    const gtin14 = barcode.padStart(14, '0');
    idx.exact.set(gtin14, name);
    
    // Store GTIN-13 (without leading zero if GTIN-14 starts with 0)
    ...
    if (noLeadingZeros.length >= 6) {
      idx.exact.set(noLeadingZeros, name);
    }
    
    // Last 8 digits for fuzzy matching
    if (barcode.length >= 8) {
  ...
  console.log('Master index built:', idx.exact.size, 'exact entries,', idx.all.length, 'products');
  return idx;
}
...
function matchProduct(gtin14, gtin13) {
  ...
  // Try exact match on GTIN-14
  if (idx.exact.has(gtin14)) {
    return { name: idx.exact.get(gtin14), type: 'EXACT' };
  }
  
  // Try exact match on GTIN-13
  if (idx.exact.has(gtin13)) {
    return { name: idx.exact.get(gtin13), type: 'EXACT' };
  }
  
  // Try without leading zeros
  const noLeadingZeros14 = gtin14.replace(/^0+/, '');
  if (noLeadingZeros14.length >= 6 && idx.exact.has(noLeadingZeros14)) {
    return { name: idx.exact.get(noLeadingZeros14), type: 'EXACT' };
  }
  
  const noLeadingZeros13 = gtin13.replace(/^0+/, '');
  if (noLeadingZeros13.length >= 6 && idx.exact.has(noLeadingZeros13)) {
    return { name: idx.exact.get(noLeadingZeros13), type: 'EXACT' };
  }
  ...
  // SEQ-6 matching (any 6-digit sequence)
...
async function saveEdit() {
  ...
  // SAVE TO MASTER DATABASE with ALL barcode variations
  // So next time ANY format of this barcode is scanned, it will be recognized!
  if (name) {
    const barcodesToSave = [];
    
    // Save GTIN-14 (padded)
    if (item.gtin14) {
      barcodesToSave.push(item.gtin14);
    }
    
    // Save GTIN-13 (without leading zero)
    if (item.gtin13) {
      barcodesToSave.push(item.gtin13);
    }
    
    // Save raw barcode from original scan
    if (item.raw) {
      const rawDigits = item.raw.replace(/\D/g, '');
      if (rawDigits.length >= 8 && rawDigits.length <= 14) {
        barcodesToSave.push(rawDigits);
      }
    }
    
    // Save all unique barcodes
    const uniqueBarcodes = [...new Set(barcodesToSave)];
    for (const barcode of uniqueBarcodes) {
      await DB.addMaster({ barcode: barcode, name: name, rms: rms });
      console.log('Saved to master:', barcode, '→', name);
    }
    
    // IMPORTANT: Rebuild the master index immediately!
...
async function init() {
  ...
  try {
    ...
    // Load pre-loaded master data (only if database is empty)
