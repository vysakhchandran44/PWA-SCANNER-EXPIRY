/**
 * Expiry Tracker v5.2.0 COMPLETE
 * Modern PWA for pharmacy expiry tracking
 * Features: GS1 parsing, GTIN-RMS matching, camera scanner, auto-save, BULK PASTE, API lookups
 * By VYSAKH
 */
...
const State = {
  ...
  currentFilter: 'all',
  searchQuery: '',
  bulkProcessing: false  // For bulk paste feature
};
...
// ========================================
async function init() {
  console.log('Expiry Tracker v5.2.0 with Bulk Paste & API integration initializing...');
  ...
  try {
    ...
    console.log('App ready with Bulk Paste & API integration!');
  } catch (err) {
...
// ========================================
async function processBulkBarcodes(text) {
  if (!text || !text.trim()) {
    showToast('Please paste barcodes', 'warning');
    return;
  }
  ...
  // Also support comma or semicolon separated
  const barcodes = [];
  ...
  if (barcodes.length === 0) {
    showToast('No valid barcodes found', 'warning');
    showLoading(false);
    State.bulkProcessing = false;
    return;
  }

  console.log(`Bulk processing ${barcodes.length} barcodes...`);
  ...
  updateBulkProgress(0, barcodes.length);

  // Process each barcode (skip API for speed during bulk)
    ...
    try {
      ...
      if (!parsed.gtin) {
        ...
        if (digits.length >= 5) {
          ...
          results.failed++;
          results.items.push({ code, status: 'failed', reason: 'Invalid barcode' });
          results.processed++;
          continue;
        }
      }

      // Match product (LOCAL only for speed)
      const match = matchProduct(parsed.gtin);
      ...
      const entry = {
        ...
        name: match.name || 'Unknown Product',
        rms: match.rms || '',
        matchType: match.name ? 'LOCAL' : 'NONE',
        expiry: parsed.expiry,
        expiryISO: parsed.expiryISO,
        expiryDisplay: parsed.expiryDisplay,
        batch: parsed.batch,
      ...
      results.success++;
      results.items.push({ code, status: 'success', entry });
    } catch (error) {
      results.failed++;
      results.items.push({ code, status: 'failed', reason: error.message });
    }
    
    results.processed++;
    updateBulkProgress(results.processed, barcodes.length);
    
    // Small delay every 10 items to prevent UI freeze
    if (i % 10 === 0 && i > 0) {
  ...
  console.log(`Bulk complete: ${results.success} success, ${results.failed} failed`);
}
...
function showBulkResults(results) {
  ...
  const html = `
    <div class="bulk-results-card">
      <div class="bulk-results-header">
        <div class="bulk-results-title">✅ Bulk Processing Complete</div>
      </div>
      <div class="bulk-results-stats">
        <div class="bulk-stat success">
          <div class="bulk-stat-value">${results.success}</div>
          <div class="bulk-stat-label">Success</div>
        </div>
        <div class="bulk-stat failed">
          <div class="bulk-stat-value">${results.failed}</div>
          <div class="bulk-stat-label">Failed</div>
        </div>
        <div class="bulk-stat total">
          <div class="bulk-stat-value">${results.total}</div>
          <div class="bulk-stat-label">Total</div>
        </div>
      </div>
      ${results.failed > 0 ? `
      <div class="bulk-failed-list">
        <div class="bulk-failed-title">❌ Failed Items:</div>
        ${results.items.filter(i => i.status === 'failed').slice(0, 10).map(item => `
          <div class="bulk-failed-item">
            <code>${escapeHtml(item.code)}</code>
            <span>${escapeHtml(item.reason)}</span>
          </div>
        `).join('')}
        ${results.failed > 10 ? `<div class="bulk-more">... and ${results.failed - 10} more</div>` : ''}
      </div>
      ` : ''}
    </div>
  `;
  ...
  // Auto-hide after 10 seconds if all successful
  if (results.failed === 0) {
    setTimeout(() => {
...
console.log('✅ Bulk Paste & API integration loaded!');
